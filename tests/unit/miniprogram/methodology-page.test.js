...
describe('miniprogram methodology page', () => {
  ...
  test('covers the same user-facing methodology sections as web', () => {
    ...
    expect(js).toContain('计算依据精简 v2');
    expect(js).toContain('候选载体');
    expect(js).toContain('本地云层展开');
    expect(js).toContain('云厚比例折损 v2');
    expect(js).toContain('内部诊断项不再作为用户可见凭据堆叠');
    expect(js).toContain('本地云层只展开中高云画布、区间分、云种修正和云厚修正');
    ...
    expect(js).toContain('localCloud, sunsetDirection, aerosolWeakCarrier');
    expect(js).toContain('受光亮度');
    expect(js).toContain('光路已经并入亮度');
    expect(js).toContain('无火烧云 <40');
    expect(wxml).toContain('2026.06.17 · 计算依据精简 v2');
