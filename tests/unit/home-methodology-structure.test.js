...
describe('home methodology structure', () => {
  ...
  test('documents the current additive carrier and light gate scoring formula', () => {
    ...
    expect(zh).toContain('最终分 = clamp(载体分 × 光路门控 + 显色修正/衰减, 0, 100)');
    ...
    expect(en).toContain('Final score = clamp(carrier × light-path gate + rendering adjustment/attenuation, 0, 100)');
  });
