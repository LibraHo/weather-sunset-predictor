...
function buildWeatherDataFromHourly(selected, hourly, selectedIdx) {
  ...
  for (let i = Math.max(0, selectedIdx - 6); i <= selectedIdx; i += 1) {
    recentPrecipitation6h += Number(hourly[i]?.precipitation || 0);
  }
...
router.post('/enhanced', validatePredictionRequest, async (req, res) => {
  try {
    const { date, lat, lon, type, options = {}, referenceTime } = req.body;

    console.log(`[PredictionRoute] Enhanced closed-loop prediction request: lat=${lat}, lon=${lon}, type=${type}`);

    const closedLoop = await buildClosedLoopPredictionInput({ lat, lon, date, type, referenceTime });
