...
function scoreDirectionalCurtainCarrier(remoteCloudData, lightPathScore = {}, weatherData = {}) {
  ...
  if (score >= 55 && upperSignal >= 75 && lowMidBlock <= 24 && nearLowMidBlock <= 30) {
    floor = parseFloat(clamp(score + 4, 60, 64).toFixed(1));
  } else if (score >= 42) {
...
function assessAerosolHazeCap(weatherData) {
  ...
  if (hasUpperCloudCarrier && severeHaze && opticalHazeOnly) {
    return { applied: true, cap: 68, level: 'optical', reason: 'optical_haze_carrier_cap_68', metrics: { aod, pm25, pm10, dust, visibility } };
  }
