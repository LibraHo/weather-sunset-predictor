...
1. **首页统一聚合接口**
   - 网页端和微信小程序首页查分必须优先调用同一个后端聚合接口：`GET /api/prediction/home`。
   - 该接口一次返回同一地点、同一日期窗口内的 `weather`、当前主预测、朝霞/晚霞卡片、3 天朝晚霞列表和性能 profile。
   - 同一地点、同一天、同一朝霞/晚霞类型在网页端和小程序端必须展示同一份后端评分结果；前端不得各自重新组织天气输入再算一遍。
   - 小程序可以先展示基础天气读取态或已返回的天气信息，但预测分数、日出日落参考时间、云层分析和文字结论必须以后端聚合结果为准。
   - 当聚合接口慢返回时，前端应保持已有读取 UI，不应阻塞基础天气卡片；聚合结果回来后，应同步刷新天气卡和预测卡，避免“天气来自旧接口、分数来自新接口”的混合状态。

2. **后端闭环批量预测 fallback**
   - 前端不逐条调用 `/api/prediction/enhanced`。
   - 只有当 `/api/prediction/home` 不可用时，才允许退回 `/api/prediction/enhanced/closed-loop/batch`，一次提交同一地点下多个 `date/type/referenceTime`。
...
4. **降级边界**
   - 首页聚合接口失败时，网页端和小程序端允许退回 closed-loop batch。
