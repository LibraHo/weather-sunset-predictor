...
Page({
  data: {
    ...
    heroEyebrow: '霞客方法',
    heroTitle: '火烧云计算方法',
    heroCopy: '霞客先算云能不能当载体，再把日落方向光路并入受光亮度，最后判断空气显色。高云多、光路通，也可能因为亮度弱而降分。',
    scoreBands: [
    ...
    formationFactors: [
      ...
      { title: '受光亮度', subtitle: '三层云亮度模型', desc: '系统会估算中高云是否真的被照亮；太阳方向光路作为其中一个亮度因子。亮度弱时按乘性系数压分，没点亮就不再给高分。' },
  ...
  applySectionMode(options = {}) {
    ...
    this.setData({
      ...
      heroCopy: apiOnly
        ...
        : '霞客先算云能不能当载体，再把日落方向光路并入受光亮度，最后判断空气显色。高云多、光路通，也可能因为亮度弱而降分。'
    });
  },
