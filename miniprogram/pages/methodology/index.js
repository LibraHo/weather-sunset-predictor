...
Page({
  data: {
    ...
    heroEyebrow: '霞客方法',
    heroTitle: '火烧云计算方法',
    heroCopy: '霞客先算云能不能当载体，再把日落方向光路并入受光亮度，最后判断空气显色。光路打开且暖霾可散射时可以保留观察价值，但当前降雨或近雨湿幕明显时不抬分。',
    scoreBands: [
    ...
    formationFactors: [
      ...
      { title: '空气显色', subtitle: '先判灰幕，再判暖散射', desc: '能见度、湿度、雨后状态和空气颗粒只改变显色质量：光路开且云幕不灰时，轻/中度 AOD、PM、dust 可增强橙红；中高云满铺且 PM/AOD 偏高时，会转为灰幕显色抑制。光路打开、暖霾可散射且没有当前降雨或近雨湿幕时，才会保留中烧观察价值。' },
    ...
    calculationSteps: [
      ...
      { title: '7. 最终显示分', formula: 'score = clamp(Σ(carrierLayer × brightnessLayer) × airRendering, 0, 100)', desc: '最终还会按硬否决校准：无火烧云 <40，轻微霞光 <60；几何不可行、厚云、满铺灰幕、湿灰幕、雨低云会进一步压制。光路打开的暖霾高云样本只有在无当前降雨和近雨湿幕时才可能保留到中烧区间。' }
    ],
    changelog: [
      { date: '2026-06-13', title: '分层亮度 + 雨天暖霾否决', summary: '最终分使用 Σ(分层载体 × 分层受光亮度) × 空气显色；光路打开、太阳方向高云强且暖霾可散射时，也必须先通过当前降雨和近雨湿幕检查。' },
  ...
  applySectionMode(options = {}) {
    ...
    this.setData({
      ...
      heroCopy: apiOnly
        ...
        : '霞客先算云能不能当载体，再把日落方向光路并入受光亮度，最后判断空气显色。光路打开且暖霾可散射时可以保留观察价值，但当前降雨或近雨湿幕明显时不抬分。'
    });
  },
