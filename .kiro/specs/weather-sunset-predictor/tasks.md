...
- [x] 2026-03-20 增量：Leaflet 中国火烧云地图锁定为静态视图（zoom=4 固定，不可拖拽/滚轮/双击缩放），并设置大陆边界 `fitBounds + setMaxBounds`
- [x] 经纬度统一投影到固定比例，保证县域级可读
- [x] `spots=[]` 时不隐藏模块，显示"今日暂无可见火烧云点位"（2026-03-20，本次增量已落地）
- _关联需求：37.4, 37.9_

#### 64.10 地图交互体验修复（2026-03-20）

- [x] 2026-03-20 修复：地图支持拖拽平移（上下左右移动），启用 dragging=true
- [x] 2026-03-20 修复：调整默认比例尺，zoom 从 4 提升到 5，避免"点位太小/范围过大看不清"
- [x] 2026-03-20 修复：保持禁用缩放（minZoom=5, maxZoom=5, zoomControl=false）
- [x] 2026-03-20 修复：禁用滚动缩放（scrollWheelZoom=false）、双击缩放（doubleClickZoom=false）
- [x] 2026-03-20 修复：禁用移动端触摸缩放（touchZoom=false）
- [x] 2026-03-20 验证：保持朝霞/晚霞分离逻辑不被破坏
- _关联需求：37_
...
## Phase 17：天气主面板 UI 重构（用户反馈优化）

> 状态：✅ 全部完成（2026-03-20）
>
> 目标：根据用户最新反馈优化天气主面板 UI，提升整体视觉一致性与信息层级。

### 任务概览

| 编号 | 任务 | 状态 |
|------|------|------|
| 65.1 | 删除独立城市横条，将城市名并入左侧主天气卡顶部 | ✅ |
| 65.2 | 左主卡信息顺序调整：城市名 → 天气状态 → 温度 → 风向风速 | ✅ |
| 65.3 | 统一左右卡片设计语言（圆角、阴影、边框、内边距） | ✅ |
| 65.4 | 增强右侧四个指标卡权重（标题与数值层级、数值字号） | ✅ |
| 65.5 | 压缩无效留白，减少割裂感 | ✅ |
| 65.6 | 板块间增加统一垂直间距 | ✅ |
...
- 修改文件：`index.html`
- 移除 `.weather-location-block` 容器
- 新增 `.weather-location-inline` 样式，置于 `.weather-visual-panel` 顶部
- **验证**：城市名显示在左侧主卡顶部，无独立横条

#### 65.2 左主卡信息顺序调整 ✅

- 修改文件：`index.html`
- 顺序：城市名 → 天气图标 → 天气描述 → 温度 → 风向风速
- **验证**：信息按预期顺序排列
...
- 圆角：统一为 `16px`（左右卡片一致）
- 阴影：统一阴影效果 `0 2px 8px rgba(0, 0, 0, 0.08)` + hover 增强
- 边框：统一厚度 `1px`
- 内边距：统一为 `12px 10px`
- **验证**：左右卡片设计语言一致
...
- 修改文件：`styles/main.css`
- 标题：字重提升至 `600`，颜色对比度增强
- 数值：字号从 `1.2rem` 提升至 `1.4rem`
- 悬停效果：增加 `transform: translateY(-2px)` 动画
- **验证**：标题与数值层级清晰，数值更醒目

#### 65.5 压缩无效留白 ✅

- 修改文件：`styles/main.css`
- 删除 `.weather-location-block`（独立城市横条）
- 调整 `.weather-glass-panel` padding：`18px → 16px`
- 调整 `.weather-visual-panel` padding：`18px 14px → 14px 12px 12px 12px`
- 调整 `.weather-feature-item-compact` 高度：`78px → 72px`
- 调整 `.weather-main-layout` gap：`14px → 12px`
- 调整 `.weather-visual-panel` 对齐方式：`center → flex-start`
- **验证**：留白减少，内容区更像整体面板

#### 65.6 板块间增加统一垂直间距 ✅

- 修改文件：`styles/main.css`
- 新增规则：`main > section { margin-bottom: var(--spacing-lg); }`
- **验证**："搜索 / 天气信息 / 朝晚霞预测"板块间有统一间距
