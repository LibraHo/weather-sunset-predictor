...
- Web 预测卡和小程序预测卡在分享按钮旁提供反馈入口。
- Web 首页菜单提供登录后反馈页，可手动填写日期、地点、朝霞/晚霞并抓取预测快照。
- 后端提供反馈提交、图片保存、后台列表/详情/图片查看接口。
- 后台反馈管理展示反馈类型、评分、图片、评论、联系方式、预测快照和天气快照。
- 验收：卡片反馈窗口校验、最多 2 张图片限制、首页未登录提示、后台详情可读、相关 JS 语法和测试通过。
...
- 将固定数量载体协同替换为两级连续模型：区域内中高云分层协同 + 区域间重叠感知协同。
- 太阳方向云幕与同源远程层采样必须合并；侧向扇区按方位分离度计算独立性；breakdown 输出分层与空间协同明细。
- 校准验收：北京朝霞 75-80、真实案例库锚点全部通过、灰幕/稀疏云/雨湿/阻塞/沙尘反例不抬高。
