...
- [ ] 54.14 用户照片审核模型：照片索引新增 `reviewStatus/reviewNote/reviewedAt/reviewedBy`，旧照片兼容为 approved；用户上传默认 pending，后台上传默认 approved。
- [ ] 54.15 用户图片管理 API：新增 `/api/photos/mine` 列表、编辑、删除接口，全部按 `uploaderUserId === userId` 校验；公开 `/api/photos` 只返回 approved。
- [ ] 54.16 后台审核能力：后台照片列表能看到全部照片，支持通过/拒绝审核并记录备注；后台照片卡片显示审核状态。
- [ ] 54.17 用户面板：小程序上传/用户面板展示“我的上传”，显示待审核/已通过/未通过状态和拒绝原因，支持删除自己的投稿。
- [ ] 54.18 用户照片投稿测试：覆盖未登录拒绝、上传默认 pending、公开列表隐藏 pending、后台审核后公开、跨用户不可访问、用户编辑后重新待审、旧照片兼容 approved。
...
- PR D2（用户图片审核管理）：54.14-54.18，把登录用户照片投稿升级为待审核、可自管、后台审核后公开。
