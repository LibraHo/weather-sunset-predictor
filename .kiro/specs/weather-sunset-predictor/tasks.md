...
### 任务拆分
- [ ] 51.1 数据模型与 API：扩展照片元数据字段 `authorName`、`placeName`、`takenAt`、`uploadedAt` 展示规范；旧数据缺字段必须兼容；`/api/photos` 不暴露内部限额字段。
- [ ] 51.2 上传表单 UI：后台/上传页增加上传者、地点、拍摄时间输入；上传者缺省显示“网友”；地点/拍摄时间允许自动读取后手动修正。
- [ ] 51.3 自动读取：继续从 EXIF 读取 GPS 与拍摄时间；新增/接入反向地理编码，根据经纬度自动建议地点名称；自动地点仅写展示字段，不反向修改坐标。
- [ ] 51.4 时间规则：服务端记录上传时间，以服务端时间为准；展示层统一转为 Asia/Shanghai（北京时间），非北京时间用户自动转换。
- [ ] 51.5 地图聚合展示：同一地点/近距离多图按 zoom 和像素距离聚合成 stack marker，显示代表缩略图 + 数量角标；点击后展示缩略图列表/小网格。
- [ ] 51.6 Marker/Popup 细节：marker、popup、聚合列表优先使用 `thumbUrl`，无缩略图才 fallback 原图；popup 展示上传者、地点、拍摄时间、上传时间。
- [ ] 51.7 i18n 与无障碍：新增用户可见文案优先维护 `zh-CN`、`zh-TW`、`en-US`，其他语言默认英文 fallback；图片、按钮、时间文本有可读 label。
- [ ] 51.8 测试与验证：补 PhotoService/API 单测、上传表单单测、地图聚合纯函数测试；用至少 50/100/150km 半径业务测试不回归；浏览器验证移动端聚合不遮挡缩放控件。

### 建议 PR 拆分 / 分发
- PR A（元数据基础）：51.1、51.4，后端 schema/API/时间规则与旧数据兼容测试。
- PR B（上传体验）：51.2、51.3、51.7，表单字段、EXIF 拍摄时间、反向地理编码建议、i18n。
- PR C（地图聚合）：51.5、51.6、51.8，Apple 相册式 stack marker、popup 列表、缩略图优先与视觉验证。
