...
| 编号 | 任务 | 关联需求 | 状态 |
|------|------|---------|------|
| 39.1 | 首页菜单改为分页导航（Forecast / Methodology） | 26.1, 26.2, 26.3 | ⏳ 待开始 |
| 39.2 | 新增「火烧云计算方法」页面内容组件 | 26.4, 26.5 | ⏳ 待开始 |
| 39.3 | i18n 增补（zh-CN / en-US） | 26.6 | ⏳ 待开始 |
| 39.4 | 响应式与可访问性优化（Tab 语义、键盘导航） | 26.7 | ⏳ 待开始 |
...
- [ ] 在主页主内容区增加分页菜单容器
- [ ] 默认激活「预测功能页」
- [ ] 分页切换不刷新整页，仅切换内容容器可见性
- [ ] 激活态样式明确（active class + aria-selected）

#### 39.2 新增火烧云计算方法页

- [ ] 新建方法说明面板（可命名为 `MethodologyPanel`）
- [ ] 内容覆盖：中高云、低云、湿度、能见度四类评分因素
- [ ] 说明评分区间：优秀（>70）/良好（40-70）/一般（<40）
- [ ] 增加示例说明文本，帮助用户理解评分解读

#### 39.3 多语言文案同步

- [ ] 新增分页与方法页 i18n key（`zh-CN`, `en-US`）
- [ ] 其他语言先回退默认文案，后续补齐

#### 39.4 响应式与可访问性

- [ ] 分页菜单遵循 `tablist/tab/tabpanel` 语义
- [ ] 支持键盘切换（← → / Enter / Space）
- [ ] 移动端支持横向滚动分页按钮
...
> 备注：本 Phase 当前仅作为需求排期记录，不纳入本次执行范围。
