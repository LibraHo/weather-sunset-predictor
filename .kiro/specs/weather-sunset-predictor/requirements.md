...
- 用户照片投稿（2026-06-01 更新）：登录用户可以使用与后台上传一致的照片处理能力，包括 MIME 校验、大小限制、EXIF 经纬度/拍摄时间解析、手动元数据覆盖、缩略图生成和每日限额。普通用户上传后的照片默认进入 `pending` 待审核状态，未经后台审核不得出现在公开 `/api/photos`、分享地图或图库展示中。
- 用户图片管理：登录用户需要在用户面板或小程序个人/上传面板中查看自己上传的图片，至少包含待审核、已通过、未通过状态；用户只能管理自己的图片，可编辑元数据或删除投稿，不能修改审核状态。用户编辑已通过图片后应重新进入待审核状态。
- 后台照片审核：后台照片管理必须能看到所有用户投稿和后台上传图片，支持通过/拒绝审核并记录审核状态、审核时间和备注。后台直接上传的照片可默认为已通过，用户端上传必须人工审核。
...
- 用户照片投稿验收：未登录上传必须拒绝；已登录上传必须写入 `userId` 并默认 `reviewStatus=pending`；公开照片列表只返回 `approved`；用户只能列出/编辑/删除自己的照片；用户编辑后重新待审；后台能列出全部照片并通过/拒绝。
