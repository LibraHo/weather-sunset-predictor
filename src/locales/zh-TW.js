const translations = {
  ...
  "home": {
    ...
    apiAccess: {
      ...
      authTitle: '認證與額度',
      authBearer: '所有 Agent 介面都需要請求標頭：Authorization: Bearer YOUR_TOKEN。',
      authDefaultQuota: '新申請預設每日 3 次呼叫，後台可依申請用途調整。',
      authAudit: '呼叫會記錄 Token、介面、狀態碼、耗時與錯誤碼，用於排查與額度統計。',
      requestTitle: '通用請求參數',
      paramLocation: '地點名稱；也可以改用 lat + lon 精確座標。',
      paramType: 'sunrise 或 sunset；預設 sunset。',
      paramDate: 'today、tomorrow 或 YYYY-MM-DD；預設 today。',
      paramDetail: 'forecast 可傳 simple 或 full；full 會返回更多天氣因子與解釋物件。',
      responseTitle: '回應重點',
      responseScore: 'score 是 0-100 的朝霞/晚霞評分，quality 為 poor/fair/good/excellent。',
      responseWindow: 'bestViewingWindow 會給出建議觀賞時間段。',
      responseExplain: 'explain 會返回 scoreComposition、factorRelations、constraints 和自然語言解釋。',
      errorsTitle: '錯誤碼處理',
      errorAuth: '401 表示 Token 缺失或無效；403/429 表示權限或額度不足。',
      errorParams: '400 多為參數錯誤，例如 type/date/lat/lon/bbox 不合法。',
      errorUpstream: '502/503 表示天氣源或地圖快取暫不可用，建議稍後重試。',
      exampleCall: '範例呼叫',
      endpoints: '核心介面',
      forecastDesc: '返回評分、品質等級和最佳觀賞窗口。',
      explainDesc: '返回評分構成、關鍵限制和自然語言解釋。',
      geocodeDesc: '返回地點候選、座標和信賴度。',
      mapSummaryDesc: '返回指定區域內達到門檻的地圖網格摘要和高分點。',
