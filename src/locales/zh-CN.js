...
export default {
  ...
  home: {
    ...
    methodology: {
      title: '火烧云计算方法',
      intro: '当前火烧云指数先判断“有没有可显色载体”，再把太阳方向光路并入受光亮度，最后用空气显色条件修正。光路打开且暖霾可散射时会保留中烧观察价值，但雨天硬阻断不抬分。',
      versionLabel: '算法版本：2026.06.13-warm-haze-mid-glow-v1',
      versionDesc: '本版使用“Σ(分层载体 × 分层受光亮度) × 空气显色”。受光亮度基于低/中/高三层云、太阳几何、太阳方向通路、直射/散射和云厚证据估算；光路打开、太阳方向高云强且暖霾可散射时，保留中烧区间，雨天硬阻断不保底。',
      ...
      changelog: {
        latest: {
          date: '2026-06-13',
          title: '分层亮度 + 暖霾中烧校准',
          summary: '最终分使用 Σ(分层载体 × 分层受光亮度) × 空气显色；光路打开、太阳方向高云强且暖霾可散射时，保留中烧观察价值；雨天 hard block 不会被保底抬分。',
          validation: '验证：2026-06-13 北京日落从 18.4 回放到 40.9；同样样本改为正在下雨时，暖霾保底不生效。'
        },
        grayVeilDirectional: {
          date: '2026-06-06',
      ...
      sections: {
        ...
        finalFormula: {
          ...
          desc: '最终分不为单个城市或日期加特殊抬分，而是让云、光路、亮度和空气共同解释分数。光路打开时，适度颗粒可让橙红更明显；但如果云层实际亮度弱，只保留到中烧区间，不抬成高分。',
          formula: '最终分 = clamp(Σ(分层载体 × 分层受光亮度) × 空气显色, 0, 100)，再经过硬否决/厚云/灰幕校准',
          highCloudCap: '高云充足但光路被挡时，会先体现在受光亮度变弱。',
          carrier: '载体分 = max(云层画布分, 气溶胶弱载体分, 太阳方向云幕载体分)',
          ...
          statusCaps: '显示分还会按状态校准：无火烧云低于 40，轻微霞光低于 60；几何不可行、厚云、灰幕和雨低云会进一步封顶；雨天 hard block 不允许暖霾保底'
        }
      }
    }
  },
  ...
  prediction: {
    ...
    analysisConclusion: {
      ...
      ledger: {
        ...
        layerSumFormula: 'Σ(载体 × 受光亮度) = {{base}}',
        canvasPlusLightPath: 'Σ(载体 × 受光亮度)',
