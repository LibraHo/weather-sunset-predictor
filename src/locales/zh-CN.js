...
export default {
  ...
  home: {
    ...
    methodology: {
      title: '火烧云计算方法',
      intro: '当前火烧云指数按“分层载体×各自受光 → 区域内协同 → 空间重叠协同 → 空气显色 → 限制校准”展示。单片强云接近取最大值，多片低重叠且都受光的云带才会连续提高分数。',
      versionLabel: '算法说明版本：2026.07.16-layer-spatial-overlap-synergy',
      versionDesc: '本版说明与结果页计算依据一致：中云和高云分别计算载体与受光；同一区域保留分层协同，不同区域按采样重叠和方位独立度去重协同。',
      ...
      changelog: {
        latest: {
          date: '2026-07-16',
          title: '分层与空间重叠协同 v1',
          summary: '中云、高云分别计算载体×受光；同一区域两层都强时连续协同，太阳方向与同源远程采样先合并，不同方位云带按独立度协同。降水或近雨湿幕明确时关闭协同。',
          validation: '验证：北京 2026-07-16 朝霞回放 79.5；11 个历史样本全部留在观测区间；雨湿云幕、稀疏云、阻塞和沙尘反例未被误抬。'
        },
        layerBrightness: {
          date: '2026-06-13',
          title: '分层求和亮度公式 v1',
      ...
      sections: {
        cloudStructure: {
          ...
          desc: '系统在本地、太阳方向和侧向区域分别计算中云载体×中云受光、高云载体×高云受光，再进行重叠感知协同。',
        ...
        transparency: {
          ...
          formula: '分层分 = 载体 × 各自受光；区域分 = 主层 + 剩余空间 × 分层协同；基础分 = 主区域 + 剩余空间 × 重叠感知协同'
        },
        layerDiversity: {
          title: '4. 空气显色',
        ...
        finalFormula: {
          title: '8. 最终分数',
          subtitle: 'Final Score · 两级协同基础分 × 空气显色',
          desc: '最终分来自区域内分层协同和区域间重叠协同，再乘空气显色并经过状态限制。不是简单求和，也没有 80 分硬门槛。',
          formula: '最终分 = clamp(重叠感知协同基础分 × 空气显色, 0, 100)，再经过硬否决/厚云/灰幕校准',
          highCloudCap: '高云充足但光路被挡时，会先体现在受光亮度变弱。',
          carrier: '基础分 = 区域内分层协同 → 区域间重叠感知协同',
  ...
  prediction: {
    ...
    analysisConclusion: {
      ...
      baseFormula: '基础分 = 云层条件 + 受光判断',
      baseHint: '太阳方向光路已纳入云层受光判断',
      canvasHint: '高云/中云提供主要色彩载体，适度薄雾可提供弱载体；低云遮挡、厚高云和灰幕会限制可用亮度',
      lightPathHint: '太阳光是否能照到云层',
      finalFormula: '最终分 = 云层基础分 × 空气显色',
      renderingHint: '云层受光、湿度、能见度和颗粒物共同影响色彩表现',
      aerosolHint: '适度气溶胶增强橙红散射，过多则发灰',
      ledger: {
        pts: '分',
        whyThisScore: '为什么是这个分数',
        weightedFormula: '{{canvas}}×80% + {{light}}×20% = {{base}}',
        gatedFormula: '云层基础分 {{base}}',
        layerSumFormula: '云层基础分 {{base}}',
        canvasPlusLightPath: '云层基础分',
        ...
        details: {
          cloudCarrier: '本地或周边是否有可被日落染色的云面',
          ...
          directionalSamples: '主光路按太阳方向判断；侧向可视云带只作为云层和受光参考',
          lightPathScoreEvidence: '太阳方向光路 {{light}} 已纳入云层受光判断',
