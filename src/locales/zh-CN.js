...
export default {
  ...
  home: {
    ...
    apiAccess: {
      ...
      authTitle: '认证与额度',
      authBearer: '所有 Agent 接口都需要请求头：Authorization: Bearer YOUR_TOKEN。',
      authDefaultQuota: '新申请默认每日 3 次调用，后台可按申请用途调整。',
      authAudit: '调用会记录 Token、接口、状态码、耗时和错误码，用于排查与额度统计。',
      requestTitle: '通用请求参数',
      paramLocation: '地点名称；也可以改用 lat + lon 精确坐标。',
      paramType: 'sunrise 或 sunset；默认 sunset。',
      paramDate: 'today、tomorrow 或 YYYY-MM-DD；默认 today。',
      paramDetail: 'forecast 可传 simple 或 full；full 会返回更多天气因子和解释对象。',
      responseTitle: '响应重点',
      responseScore: 'score 是 0-100 的朝霞/晚霞评分，quality 为 poor/fair/good/excellent。',
      responseWindow: 'bestViewingWindow 给出建议观赏时间段。',
      responseExplain: 'explain 会返回 scoreComposition、factorRelations、constraints 和自然语言解释。',
      errorsTitle: '错误码处理',
      errorAuth: '401 表示 Token 缺失或无效；403/429 表示权限或额度不足。',
      errorParams: '400 多为参数错误，例如 type/date/lat/lon/bbox 不合法。',
      errorUpstream: '502/503 表示天气源或地图缓存暂不可用，建议稍后重试。',
      exampleCall: '示例调用',
      endpoints: '核心接口',
    ...
    methodology: {
      ...
      sections: {
        ...
        layerDiversity: {
          ...
          oneLayer: '光路<50：门控 0.25–0.65；太阳方向云墙会进一步压到约 0.42/0.55'
        },
        lowCloudPenalty: {
          title: '5. 低云惩罚系数',
