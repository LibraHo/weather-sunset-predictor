...
class GeocodingServiceFactory {
  ...
  static create(proxyURL) {
    ...
    // 使用相对路径，部署后自动指向正确的后端
    const defaultProxyURL = window.location.origin;
    const storedProxyURL = localStorage.getItem('api_proxy_url') || defaultProxyURL;
    const effectiveProxyURL = proxyURL || storedProxyURL;

    console.log(`[GeocodingServiceFactory] 后端代理 → ${provider}, URL=${effectiveProxyURL}`);
